...
1. c(35, 46, 17, 58,2)
```{r}
quickfx <- function(my_object = c(35, 46, 17, 58,2)){
  res <- lapply(list(identity, class, typeof), function(x) x(my_object))
  names(res) <- c("object", "class", "type")
  print(res)
}
quickfx()
```
...
```{r}
quickfx(cbind (c(1,0,0), c(0,1,0), c(0,0,1)))
```

5. cbind (c("a", "b", "c"), c (1,2,3))
```{r}
quickfx(cbind (c("a", "b", "c"), c (1,2,3)))
```
6. data.frame (var1 = 1:10, var2 = letters[1:10], var3 = rep(1:2,5), stringsAsFactors = FALSE )
```{r}
quickfx(data.frame (var1 = 1:10, var2 = letters[1:10], var3 = rep(1:2,5), stringsAsFactors = FALSE ))
```
